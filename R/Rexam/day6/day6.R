...
as.Date('110228',format='%d%b%y') #서식에 안 맞기 때문 NA #b가아니라 m이어야함 
as.Date('110228',format='%y%m%d') 

x1 <- "2019-01-10 13:30:41" #날짜-시간 규격 문자열 
# 문자열을 날짜형으로
...
as.POSIXlt("2021/03/02")$wday #lt의 장점 
...
format(tmp,'오늘은 %Y년 %B %d일 %A입니다') #포맷팅

# 정규표현식 -> a,b,c,d...,z --> [a-z] 가,각,갂,,,힣 --> [가-힣]

word <- "JAVA javascript 가나다 123 %^&*"
gsub("A", "", word) #단어 대체, gsub는 그 단어마다, sub는 맨 앞 한번만
gsub("a", "", word) 
gsub("[Aa]", "", word) 
gsub("[가-힣]", "", word) #정규표현식 사용하여 제거 
gsub("[^가-힣]", "", word) #^ -> 부정표현 
gsub("[&^%*]", "", word) 
gsub("[[:punct:]]", "", word) 
gsub("[[:alnum:]]", "", word) #블랭크, 특수문자만 남음 (문자,숫자)
...
gsub("Aa", "", word) 
gsub("(Aa)", "", word) 
gsub("(Aa){2}", "", word);gsub("Aa{2}", "", word) #Aa 2번 나온애들만 삭제해라.. 
...
gsub("[1234567890]", "", word) 
gsub("[0-9]", "", word) 
gsub("\\d", "", word); gsub("\\D", "", word) #숫자제거, 숫자만 남기기
gsub("[[:digit:]]", "", word) 
gsub("[^[:alnum:]]", "", word) 
gsub("[[:space:]]", "", word) 
gsub("[[:punct:][:digit:]]", "", word) 
gsub("[[:punct:][:digit:][:space:]]", "", word) #or
...
landmesses
grep(pattern="New", x=landmesses)#New 들어가 있는 위치를 내보냄
...
gsub("ABC", "@", "123AunicoBC98ABC") #ABC붙어있는것만
...
grep("che", words, value=T) #대괄호가 없으면 단어통째, 있으면 그 문자열 하나하나
...
grep("^c", words, value=T) #[^...] -> 부정, 대괄호 없으면 C로 시작하는 문자열 의미
grep("t$", words, value=T) #t로 끝나는 문자열
...
fox.said
strsplit(x= fox.said, split= " ") #문자열 쪼개기
...
?apply
?lapply #결과가 리스트
apply(df, 1, sum, na.rm=TRUE) #na 제거
apply(df, 2, sum, na.rm=TRUE)
lapply(df, sum, na.rm=TRUE) #리스트의 합은 열의 합이 보통(열단위 기준)
sapply(df, sum, na.rm=TRUE) #결과가 심플데이터셋
tapply(1:6, gender, sum) # rm(sum) #gender에 1:6까지 대입, 계산
tapply(df$w, gender, mean, na.rm=TRUE)
mapply(paste, 1:5, LETTERS[1:5], month.abb[1:5])#인자의 길이가 모두 같아야함
...
sapply(flower, function(d, n=5) if(nchar(d) > n) return(d), 3) #sapply(flower,3)
sapply(flower, function(d, n=5) if(nchar(d) > n) return(d), n=4)  #flower=d, 3=n 이다
