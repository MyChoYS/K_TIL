...
plot(국어, type="o", col="blue", ylim=c(0,10), axes=FALSE, ann=FALSE) #axes,ann = FALSE라 x축 y축 안보임
...
#산포도 
(성적 <- read.table("data/성적.txt", header=TRUE)) #첫번째 행 -> header=true로 
...
legend(11,30, names(성적1), cex=0.8, fill=rainbow(3)); #범례
...
data <- read.table("data/온도.txt", header=TRUE, sep=",") #sep! -> read.csv 해도 됨 ","가 있기 때문 
head(data, n=5) 
dim(data) #행 100개 열 12개 
View(data)
boxplot(data)
boxplot(data, las = 2)
...
chtcols
boxplot(data, las = 2, at = c(1,2,3,4, 6,7,8,9, 11,12,13,14), col=chtcols) #색깔 4개씩 묶어서 
