...
sapply(v, evenodd1) #vector
lapply(v, evenodd1) #list로 리턴
...
print(ft(100))
print(a);print(b);print(c);print(d) #d는 지역변수이므로 객체d를 찾을 수 없다
...
#dataframe이 list에, list는 vector에 포함 됨 but, list는 데이터저장에 제한 x
#list는 원소 1개 짜리 1차원 배열과 같다.          vector는 같은 종류만 저장o
testParamType(100)
...
testParamType1 <- function(x){
  result <- NULL #데이터 값이 모두 밑의 조건에 해당되지 않을 경우를 대비
  if(is.vector(x)  && !is.list(x)) result <-"벡터를 전달했군요!" #벡터만 구분
  else if(is.data.frame(x)) result <- "데이터프레임을 전달했군요!"
  else if(is.list(x)) result <- "리스트를 전달했군요!" #리스트는 벡터이기도 함
...
#try() 함수를 알아야 데이터 수집을 편하게 할 수 있다.!! 에러가 나더라도 다음
test2 <- function(p){
  cat("난 수행함\n")
  try(testError1(-1)) #testError(-1)
  cat("나 수행할 까요? \n")
}
test2() ##textError에는 stop이 있음에도 불구하고, 오류가 나더라도 밑의 코드 실행

testAll <-function(p){
  tryCatch({ #파이썬의 try,except finally 구문과 비슷
    if(p=="오류테스트"){
      testError1(-1)
    }else if (p =="경고테스트"){
      testWarn(6)
...
f.case2 <- function(x) {
  if(any(is.na(x))) #any -> 하나만 true여도 true 
    return("NA가 있슈")
  else
    return("NA가 없슈")
}
...
f.case3 <- function(x) {
  if(all(is.na(x))) #all() -> 모두 true이어야 true
    return("모두 NA임")
  else
    return("모두 NA인 것은 아님")
}
...
str(emp)
save(list=c('emp'), file = "two.rda") #특정 원하는 객체만 저장
rm(emp)
load("two.rda") #저장은 .rda로 할 것
